...
def render_model_performance_page():
    ...
    st.markdown(f"""
    <div style="background:#1a1d2e;border:1px solid #2a2d3e;border-radius:12px;padding:16px 18px;margin:14px 0">
        <div style="font-size:12px;color:#666;font-weight:600;letter-spacing:0.06em;text-transform:uppercase;margin-bottom:8px">
            Cross-Validation Stability (5-fold TimeSeriesSplit)
        </div>
        <div style="font-size:24px;font-weight:700;color:#4c9be8">
            {cv_mean:.1f}% <span style="font-size:14px;color:#888;font-weight:400">± {cv_std:.1f}%</span>
        </div>
    ...
    with col_gauge:
        fig = go.Figure(go.Indicator(
            ...
            title={"text": "Model Accuracy vs Random Baseline (50%)", "font": {"color": "white", "size": 13}},
        ))
        fig.update_layout(paper_bgcolor="#0e1117", font_color="white", height=260,
    ...
    with col_bench:
        ...
        for label, val, color in benchmarks:
            ...
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
                <div style="width:140px;font-size:12px;color:{'white' if is_yours else '#888'};font-weight:{'700' if is_yours else '400'}">{label}</div>
                <div style="flex:1;background:#13151f;border-radius:4px;height:18px;position:relative">
    ...
    if features:
        feat_html = "".join([
            f"<span style='background:#1a2a3a;color:#4c9be8;padding:4px 10px;border-radius:6px;font-size:12px;margin:3px;display:inline-block'>{f}</span>"
            for f in features
        ])
    ...
    st.markdown(f"""
    <div style="background:#1a1d2e;border-left:3px solid {color};border-radius:8px;padding:12px 16px;font-size:13px;color:#ccc">
        {interp}
    </div>
    """, unsafe_allow_html=True)